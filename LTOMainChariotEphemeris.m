function [State, aop, mf, tSpiral] = LTOMainChariotEphemeris(System, State, Spacecraft, Option)
...
while isFeasible && ~Option.doneFeasible
	...
	[Problem, State] = setProblemfsolveChariotEphemeris(System, State, Spacecraft, Option, ...
		Collocation);
	...
	aop = feasibleVec(end-2);
	mf = feasibleVec(end-1);
	tSpiral = feasibleVec(end);
	Option.SpiralDown.mf = mf;
	Option.SpiralDown.aop = aop;
	Option.SpiralDown.time = tSpiral;
...
while isOptimize && ~Option.doneOptimize
	...
	[x0, funcs, options, State] = setProblemipoptChariotEphemeris(System, State, Spacecraft, Option, Collocation);
	...
	aop = optimizedVec(end-2);
	mf = optimizedVec(end-1);
	tSpiral = optimizedVec(end);
	Option.SpiralDown.aop = aop;
	Option.SpiralDown.mf = mf;
	Option.SpiralDown.time = tSpiral;
% 	closestEncounter(System, State);
