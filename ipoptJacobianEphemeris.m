function dFspr = ipoptJacobianStructureEphemeris(x, auxdata)
...
for iPhase = 1:nPhase
	...
	for jj = 1:28
		...
		Yp = Y + h*1j*hMat;
		Ydotp = getDerivCSIVectorizedEphemeris(muND, Yp, UVar, ispND, g0ND, bodyStateMatVar, iCB);
		Cp = mtimesx([Yp, dtMat/2.*Ydotp], invA);
		CBp = mtimesx(Cp,B);
		CDp = mtimesx(Cp,D);
		CBdotp = getDerivCSIVectorizedEphemeris(muND, CBp, UDef, ispND, g0ND, bodyStateMatDef, iCB);
	...
	for jj = 1:4
		...
		Ydotp = getDerivCSIVectorizedEphemeris(muND, Y, UVarp, ispND, g0ND, bodyStateMatVar, iCB);
		
		Cp = mtimesx([Y, dtMat/2.*Ydotp], invA);
		CBp = mtimesx(Cp,B);
		CDp = mtimesx(Cp,D);
		CBdotp = getDerivCSIVectorizedEphemeris(muND, CBp, UDefp, ispND, g0ND, bodyStateMatDef, iCB);
		FMatp = CDp - dtMat/2.*CBdotp;
	...
	dFDefect = [dFdx, dFdu, zeros(mDefect, nxOpt(iPhase)-nState-nControl)];
