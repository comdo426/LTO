...
phaseBody{2,1} = {'SUN', 'EARTH'};
phaseBody{3,1} = {'SUN', 'MARS'};
...
load('InitialGuessChariot.mat')
% load('InitialGuessChariotSmall.mat')
...
Option.SpiralDown.time = 0.261888646622344; % n.d. time w.r.t. the prop. time
% Option.SpiralDown.integrate = odeset(Option.integrate, 'Events', ...
% 	@(t,y) JCTargetFunc(t,y,System{3}.parameter.mu, JCTarget));
Option.SpiralDown.integrate = Option.integrate;
...
Option.ipoptopt.ipopt.acceptable_tol = 5e-6;
Option.ipoptopt.ipopt.acceptable_iter = 15;

% Option.ipoptopt.ipopt.tol = 1e-2;
% Option.ipopt.ipopt.acceptable_tol = 1e-2;

% Option.ipoptopt.ipopt.constr_viol_tol = 1e-11;
% Option.ipoptopt.ipopt.acceptable_constr_viol_tol = 1e-10;
% 
% Option.ipoptopt.ipopt.dual_inf_tol = 1e-5;
% Option.ipoptopt.ipopt.acceptable_dual_inf_tol = 1e-5;
% 
% Option.ipoptopt.ipopt.compl_inf_tol = 1e-4;
% Option.ipoptopt.ipopt.acceptable_compl_inf_tol = 1e-4;
...
Option.aopBoundary = 1; % rad;
Option.mfBoundary = 0.1; % n.d.
Option.tSpiralBoundary = 0.1;
...
[Result, aopConverged, mfConverged, tConverged] = LTOMainChariot(System, InitialGuess, Spacecraft, Option);
...
figure(SEPlotNo)
for iSegment = 1:Result{2}.nSegment
	isThrust = Result{2}.control(iSegment, 1) > Spacecraft{2}.thrustMaxND*0.05;
	if isThrust
		plot3(Result{2}.state(4*(iSegment-1)+1:4*(iSegment-1)+4,1), ...
			Result{2}.state(4*(iSegment-1)+1:4*(iSegment-1)+4,2), ...
			Result{2}.state(4*(iSegment-1)+1:4*(iSegment-1)+4,1), ...
			'r', 'linewidth', 1.0);
	else
		plot3(Result{2}.state(4*(iSegment-1)+1:4*(iSegment-1)+4,1), ...
			Result{2}.state(4*(iSegment-1)+1:4*(iSegment-1)+4,2), ...
...
SESCI = rot2inert(Result{2}.state, Result{2}.timeVariable*tstarSE, 0, SEFrameSC, Result{2}.t0);
